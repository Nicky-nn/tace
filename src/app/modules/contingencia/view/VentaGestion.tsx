...
const tableColumns: MRT_ColumnDef<FacturaProps>[] = [
  ...
  {
    header: 'Tipo Emisión',
    accessorKey: 'tipoEmision.descripcion',
    accessorFn: (row) => {
      if (row.tipoEmision && row.tipoEmision.descripcion) {
        return (
          <Chip
            color={
              row.tipoEmision.descripcion === 'FUERA DE LINEA' ? 'warning' : 'success'
            }
            label={row.tipoEmision.descripcion}
            icon={
              row.tipoEmision.descripcion === 'FUERA DE LINEA' ? (
                <LayersClear />
              ) : (
                <Mail />
              )
      ...
      // Si falta algún valor, puedes mostrar un mensaje alternativo o simplemente devolver `null`.
      return null
    },
  },
  {
        ...
        <div className="breadcrumb">
          <Breadcrumb
            routeSegments={[
              ...
              { name: 'Gestión Ventas de Entidades Financieras' },
            ]}
          />
        </div>
